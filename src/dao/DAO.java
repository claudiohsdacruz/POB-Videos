...
public abstract class DAO<T> implements DAOInterface<T> {
	...
	public static void open(){
		if(manager==null){
			...
			if(sgbd.equals("mysql"))
				prop.setProperty("javax.persistence.jdbc.url", "jdbc:mysql://"+ip+":3306/videos?createDatabaseIfNotExist=true");
	...
	public static void clear() {
		...
		q = manager.createQuery("delete from Visualizacoes");
		cont =  q.executeUpdate();
		log.debug("deletou visualizacoes: "+ cont);

		q = manager.createQuery("delete from Videos");
		cont =  q.executeUpdate();
